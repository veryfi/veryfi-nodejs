...
//Creating the Client
const veryfi_client = new Client(
    client_id,
    client_secret,
    username,
    api_key,
...
describe('Processing documents', () => {
    test('Upload invoice for processing', async () => {
        const response = await veryfi_client.process_document(
    ...
    test('Process document from URL', async () => {
        const response = await veryfi_client.process_document_url(
    ...
    const checkReceiptResponse = (response: VeryfiDocument) => {
        ...
        expect(response.vendor?.address).toBe(
    ...
    const checkInvoiceResponse = (response: VeryfiDocument) => {
        ...
        expect(response.vendor?.address).toBe(
            '3800 Struble Road, Cincinnati, Ohio 45251, United States',
        );
...
describe('Editing Documents', () => {
    test("Update a document's entries", async () => {
        ...
        const params = { notes: randomString };

        const docs = await veryfi_client.get_documents();
        const doc_id = docs.documents[0].id;
        const response = await veryfi_client.update_document(doc_id, params);
    ...
    test('Delete a document by id', async () => {
        ...
        const doc_id = docs.documents[0].id;
        await expect(
            veryfi_client.delete_document(doc_id!),
...
describe('Process w2 documents', () => {
    test('Process a document from file_path', async () => {
        const doc = await veryfi_client.process_w2_document(
            'resources/w2.png',
            true,
        );
    ...
    test('Get documents and get a document by id', async () => {
        const docs = await veryfi_client.get_w2_documents();
        expect(docs.length).toBeGreaterThan(1);
        const doc_id = docs[0].id;
        const doc = await veryfi_client.get_w2_document(doc_id!);
        expect(doc['id']).toBe(doc_id);
    });
    test('Get documents with page', async () => {
        const docs = await veryfi_client.get_w2_documents(3);
        expect(docs.length).toBeGreaterThan(1);
    });
    test('Process a document from url', async () => {
        const doc = await veryfi_client.process_w2_document_from_url(
            'w2.png',
...
describe('Test bad credentials', () => {
    test('Test bad credentials', async () => {
        const veryfi_wrong_client = new Client(
            'client_id',
            'client_secret',
            'username',
            'api_key',
